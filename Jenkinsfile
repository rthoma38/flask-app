pipeline {
    ...
    environment {
        ...
        OUTPUT_DIR = 'artifact_reports'
        NIKTO_REPORT = 'nikto_report.txt'
        ZAP_REPORT = 'zap_report.html'
        GITLEAKS_REPORT = 'gitleaks_report.json'
        CHECKOV_REPORT = 'checkov_dockerfile_report.json'
        NIKTO_TARGET = 'http://localhost:5000'
    ...
    stages {
        stage('Checkout') {
            steps {
                git url: 'https://github.com/rthoma38/flask-app.git', branch: 'main'
            }
        }
        ...
        stage('Vulnerability Scan (Trivy)') {
            steps {
                sh 'docker run --rm -v /var/run/docker.sock:/var/run/docker.sock aquasec/trivy:latest image flask-application || true'
            }
        }
        ...
        stage('Run Checkov Scan (Dockerfile)') {
            steps {
                script {
                    sh """
                        docker run --rm \
                            ...
                            --output json > ${OUTPUT_DIR}/${CHECKOV_REPORT} || true
                    """
                }
            }
        }
        ...
        stage('Archive Reports') {
            steps {
                ...
                archiveArtifacts artifacts: "${OUTPUT_DIR}/${CHECKOV_REPORT}", allowEmptyArchive: true
            }
        }
    }
}
